...
>###### 随机访问（Random Access）：
>
>随机访问是指可以通过下标**直接访问**vector中的任何元素。vector在内存中是以**连续**的块存储数据的，这使得我们能够通过下标（如v[i]）在常数时间内访问任意元素（即O(1)时间复杂度）。这种特性是vector相比链表等数据结构的一个重要优势。
>
>例如：
>
...
>
>边界检查是指在访问容器中的元素时，程序会检查**是否访问了容器的有效范围**。这样可以**避免访问越界**，防止程序出错。
>
...
>###### 解引用（Dereferencing）：
>
>解引用是指**通过迭代器或指针来访问指向的对象或数据**。在C++中，迭代器和指针类似，都指向容器中的元素或变量，而解引用就是通过它们访问或操作该元素的值。
>
