...
从上到下分为应用层、传输层、网络层和链路层共四层

应用层只需要专注于为用户提供应用功能，比如 HTTP、FTP、Telnet、DNS、SMTP 等，它不用关心数据是如何传输的，它工作在操作系统的用户态

传输层为应用层提供网络支持，主要是有 TCP 和 UDP 两个传输协议，负责把数据包传给应用，不同应用有着不同的端口
