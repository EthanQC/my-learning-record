...
但如果查询的是 **age = 40** 的记录，由于表中上一条记录是 age =39，这也是表里的最后一条记录，那么 MySQL 会加**临键锁**，范围是 `(39, +∞]`，从而阻止其他事务插入 age 大于 39 的记录
...
#### 如何避免一条语句在没有加索引时发生全表扫描锁表的问题？

可以将 MySQL 中的 **`sql_safe_updates`** 参数设置为 1，开启**安全更新模式**，这样的话，**update** 语句必须满足如下条件之一才能执行：

* 使用 where，并且 where 条件中必须有索引列
* 使用 limit
* 同时使用 where 和 limit，此时 where 条件中可以没有索引列

**delete** 语句则必须同时使用 where 和 limit，此时 where 条件中可以没有索引列，如果 where 条件带上了索引列，但优化器最终选择的还是全表扫描而不是索引的话，我们可以使用 **`force index([index_name])`** 来强制使用索引

#### MySQL 在可重复读隔离级别下什么情况可能会出现死锁，如何在 MySQL 中避免死锁？

假设现在**有两个事务同时要对同一张表进行操作**，先检查某个字段值的记录是否存在，如果**不存在就插入这条记录**，这里在查询记录是否存在时使用的是 **`select ... for update`** 语句，是为了防止事务执行的过程中有其他事务插入了这条记录，从而导致幻读的问题

但如果两个事务同时执行了这条语句，并且查询的记录都不存在，这时两个事务都会对这张表加上一个 **X 型的相同范围的临键锁**（**查询的记录的上一条记录是表里的最后一条记录**，才会加临键锁，否则会加间隙锁），范围右边界是加在 **`supremum pseudo-record`（上确界伪记录）**上的，**所以这里加的是临键锁，但实际效果是间隙锁的效果**，而这两个事务在接下来执行插入语句时，都会在插入间隙上获取一个**插入意向锁**，插入意向锁与临键锁（间隙锁）是**冲突**的，但间隙锁与间隙锁是兼容的，所以两个事务在执行插入语句时，都会因为**对方持有的插入位置的临键锁导致获取不到插入意向锁**（插入意向锁状态是**等待**）而被阻塞，从而导致**死锁**

![deadlock-exp](deadlock-exp.png)

死锁的四个必要条件是**互斥、占有且等待、不可强占用、循环等待**，我们只要破坏任意一个条件死锁就不会成立

在**数据库层面**，我们可以**设置事务等待锁的超时时间**，在一个事务的等待时间超过这个值之后，就对这个事务进行回滚，从而释放这个锁，让另一个事务继续执行，在 InnoDB 的参数是 **`innodb_lock_wait_timeout`**，默认值是 50 秒

还可以开启**主动死锁检测**，在发现死锁之后会主动回滚死锁链条中的某一个事务，让其他事务得以继续执行，参数是 **`innodb_deadlock_detect`**，将其设置为 on 即可

在**业务层面**的话，由于使用 `select ... for update` 语句是为了做**幂等性校验**，防止**出现重复的记录**（比如重复的订单），那我们可以直接**将对应的字段设置为唯一索引列**，利用唯一索引的唯一性来保证不会重复，不过这种方式在插入重复的记录时就会**抛出异常**

#### 隐式锁是什么？

在事务需要加锁时，如果这个锁**不可能发生冲突**，InnoDB 就会**跳过**加锁环节，这就是**隐式锁**，它是一种**延迟加锁机制**，只在有可能发生冲突时才加锁，从而减少锁的数量，提高系统的整体性能

如果记录之间加有间隙锁，为了避免幻读，此时是不能插入记录的，也就是插入意向锁的状态是**等待**

如果是遇到**唯一索引冲突**，那新的插入语句除了会失败以外，还会给这条记录加 **S 型的锁**，如果是**主键索引重复**就是给**聚簇索引加记录锁**，如果是**二级索引的唯一重复**，那会给**二级索引加临键锁**

假设一个事务 A 先插入了一条记录，此时对应的唯一二级索引记录就被隐式锁保护了，但这里还没有实际的锁结构，接下来事务 B 也插入了一条同样值的记录，也就是导致了重复，这时事务 B 是想获得一个 S 型的临键锁的，但由于事务 A 还没有提交，事务 A 插入的这个记录上的隐式锁就会变成显示锁，锁类型是 X 型的记录锁，从而让事务 B 遇到锁冲突，进入阻塞状态，因为 **X 型与 S 型的锁是冲突的**

如果这里的二级索引**不是**唯一索引，那两个事务前后执行相同的插入语句，是不会发生阻塞的，但是会发生上面的死锁问题

#### 下面这个场景，在可重复读隔离级别下，会发生什么？

![deadlock-question](deadlock-question.png)

会发生**死锁**，**更新一条不存在的记录是完全合法的 SQL操作，只是受影响行数为 0**

事务 A 和 B 在 t1 和 t2 时间分别更新了一条不存在的记录，它们会分别**在 `(20, 30)` 这个范围加上 X 类型的间隙锁**，由于间隙锁的意义只在于阻止区间被插入，因此是**可以共存**的，一个事务获取的间隙锁不会阻止另一个事务获取同一个间隙范围的间隙锁，共享（S 型）和排他（X 型）的间隙锁是没有区别的

然后在 t3 和 t4 时刻事务 A 和 B 又分别要插入一条记录，在插入之前要获取插入意向锁，它是一种特殊的间隙锁，但它只用于并发插入操作，并且两个事务是不能在同一时间内，一个拥有间隙锁，另一个拥有该间隙区间内的插入意向锁的，所以会**发生冲突**，此时的插入意向锁会被设置为**等待**状态，从而阻塞插入语句

所以，**事务 A 的插入意向锁会因为事务 B 的间隙锁而被设置为等待状态，事务 B 的插入意向锁会因为事务 A 的间隙锁而被设置为等待状态**，从而发生死锁，两个事务的插入意向锁都在等待对方

而之所以会先更新不存在的记录是因为，在实际的代码逻辑中，一般都是会**先尝试更新，然后如果没有更新，说明记录不存在，才会执行插入**，这种写法在高并发的情况下就会导致死锁
