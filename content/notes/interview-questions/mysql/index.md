...
MySQL 的数据是持久化到磁盘的，当我们通过索引查找某行数据时，要先从磁盘读取索引到内存，再通过索引从磁盘中找到某行数据，然后读入到内存，也就是说在查询过程中是会发生**多次磁盘 I/O** 的，而磁盘的 I/O 次数越多，消耗的时间也就越大，所以我们希望索引的数据结构能在**尽可能少**的磁盘 I/O 中完成查询工作，另外由于 MySQL 是支持范围查找的，所以索引的数据结构还要**能高效地执行范围查找**，这些是我们的需求

首先，索引数据最好能**按照顺序排列**，这样可以使用**二分查找**来高效定位数据，比如使用一个有序数组来存储索引的话，我们可以通过二分查找在 O(log n) 的时间复杂度内定位到某个索引键，然后再通过索引键找到对应的数据行，但每次查找都需要不断计算中间位置，并且数组在插入和删除时需要**移动大量元素**，导致效率低下

于是我们可以考虑使用**非线性并且天然适合二分查找的数据结构**，比如**二叉查找树**，它的一个节点的左子树的所有节点都小于这个节点，右子树的所有节点都大于这个节点，这样我们就不需要计算中间节点的位置了，直接比较即可，并且由于二叉查找树是一个跳跃结构，不用连续排列，所以插入和删除时不需要移动大量元素，效率更高，但二叉查找树有一个问题是，如果每次插入的元素都是二叉查找树中最大的元素，它就会**退化成一条链表**，它的**高度可能会很大**，导致查找时需要经过很多层节点才能找到目标节点，从而增加了磁盘 I/O 的次数（由于树是存储在磁盘中的，每访问一个节点都对应一次磁盘 I/O，也就是说**树的高度就相当于每次查询数据时磁盘 I/O 操作的次数**）

为了解决这个问题，我们可以使用**平衡二叉查找树**（AVL 树），它的每个节点的左子树和右子树的高度差不能超过 1，或者也可以使用其他自平衡的二叉树，比如红黑树，但无论是哪种，**它们都会随着插入的元素增多而导致树的高度变高**，因为它们都是**二叉树**

为了进一步降低树的高度，我们可以考虑使用**多路平衡查找树**，比如 B 树，它的**每个节点可以有多个子节点**，它的查询效率会比平衡二叉树更高，但**它的每个节点都会包含索引 + 记录**，记录的数据大小是很有可能远远超过索引数据的，这代表我们需要花费更多的磁盘 I/O 来读到有用的索引数据，并且还会把没用的记录数据读到内存中，浪费内存空间，另外如果使用 B 树来做范围查询的话需要使用中序遍历来访问节点，效率也不是特别高

最后，在 B 树的基础上 **B+ 树**做了升级，B+ 树**只有在叶子节点上才会存放实际数据**，**非叶子节点只存放索引**，在数据量相同的情况下，它比又存索引又存记录的 B 树的非叶子节点能存放更多的索引，因此 B+ 树是可以**更加矮胖**的，查询的磁盘 I/O 次数也会更少，并且由于 B+ 树是存在冗余节点的，所以它的**插入和删除效率都会更高**，它不会像 B 树那样有复杂的树的变形，另外 B+ 树的所有层的节点之间都是用**双向链表**连接起来的，还都是按照顺序排列的，这样就能非常高效地做范围查找了，所以 MySQL 采用 B+ 树作为索引的数据结构

#### 哪些情况下索引会失效？

**对索引使用左或者左右模糊匹配：**

左或者左右模糊匹配就是 `like %xx` 或者 `like %xx%` 这种形式，由于索引是按照索引值有序排列的，而这个顺序是**从左到右的升序**，所以它**只能根据前缀进行比较**，也就是只有 `like xx%` 这种右模糊索引才能使用到索引扫描，而左模糊和左右模糊匹配会导致索引失效，从而导致全表扫描

**在索引列上使用函数：**

如果在查询条件中**对索引字段使用函数**，会导致索引失效，因为索引保存的是索引字段的**原始值**，而**不是经过函数计算后的值**，值都不一样了没法匹配了自然就没办法用索引了，除非使用了**函数索引**，也就是针对函数计算后的值建立一个索引，相当于该索引的值是函数计算后的值，所以才可以通过扫描索引来查询数据

**对索引进行表达式计算：**

如果在查询条件中对索引字段进行表达式计算，比如 `age + 1 > 18`，这样会导致索引失效，也是**同样的因为索引保存的是字段的原始值**，而不是经过表达式计算后的值，值都不一样了没法匹配了自然就没办法用索引了，正确的写法应该是 `age > 17`，把表达式计算放在右边

**对索引隐式类型转换：**

MySQL 在遇到字符串和数字进行比较时，会**自动把字符串转为数字**，然后再进行比较，如果索引列是字符串类型，而查询条件中传入的是数字类型，那么 MySQL 会把索引列的值转为数字进行比较，**相当于对索引列使用了函数操作**，从而导致索引失效，而如果索引列是数字类型，查询条件中传入的是字符串类型，那么 MySQL 会把查询条件转为数字进行比较，这样是不会导致索引失效的，因为索引的字段并没有使用任何函数

（类型转换：`CAST(phone AS signed int)` 或 `CAST("1" AS signed int)`）

**联合索引没有遵循最左前缀原则：**

联合索引要求从最左边的列开始**连续命中索引列**才能使用到索引扫描，如果查询条件中没有使用联合索引的第一列，或者中间某一列没有使用，那么索引就会失效

除非是使用了**索引下推**，例如 `where a = 1 and c = 3`（严格意义上属于索引截断）这种联合索引是 `(a, b, c)` 的情况，虽然没有命中 b 列，但可以先通过索引定位到所有 a = 1 的行，然后再**下推到存储引擎中**，在这些行中筛选出 c = 3 的行，直接在引擎层就过滤掉大量的数据，**无需再回表读取数据来进行判断**，减少了回表次数，从而提升查询效率，在执行计划中如果有 `Extra = Using index condition` 就表示使用了索引下推

**where 子句中的 or：**

在 where 子句中，如果在 or 前的条件列是索引列，而在 or 后的条件里不是索引列，那么索引就会失效，因为 or 的含义是**两个只要满足一个即可**，只有一个条件列是索引列是没有意义的，只要有条件列不是索引列，就会进行全表扫描，除非 or 前后的条件列都是索引列，这样 MySQL 会分别使用各自的索引来查找数据，然后把结果合并起来

#### count(*) 和 count(1) 有什么区别？哪个性能最好？

按照性能排序，**count(*) = count(1) > count(主键字段) > count(非主键字段)**

count() 是一个**聚合函数**，它的参数不仅可以是**字段名**，也可以是**其他任意表达式**，它主要是被**用来统计符合查询条件的记录中，函数指定的参数不为空的记录数**

在通过 count 函数统计时，MySQL 的 server 层会维护一个叫 count 的变量，server 层会循环向 InnoDB 读取一条记录，如果指定的参数不为空，就会把变量 count 加 1，最后返回这个变量的值，如果表里只有主键索引，count 还是主键的话，那 InnoDB 只需要扫描**聚簇索引**就能完成统计，如果表里有**二级索引**，那此时循环遍历的就不是聚簇索引而是二级索引了，因为相同数量的二级索引记录可以比聚簇索引记录占用**更少的空间**，从而减少磁盘 I/O，提高统计效率

在执行 count(1) 的过程中，由于参数是 1，不是字段，所以**不会读取记录中的任何字段的值，只会统计记录的数量**，而 count(*) 的执行过程中，**参数 `*` 会被 MySQL 转化成参数 0 来处理**，所以 count(*) 和 count(1) 的执行过程是一样的，都是只统计记录的数量，不会读取任何字段的值，**性能也没有什么差异**，如果有二级索引的话，MySQL 也都会会选择扫描二级索引来完成统计

如果执行 count(主键字段) 的话，会采用**全表扫描**的方式来计数，如果要统计表中某个非主键字段不为空的记录个数，最好是**给这个字段建立一个二级索引**

#### 如何优化 count(*)？

如果业务对于统计个数**不需要很精确**，例如搜索引擎在搜索关键词时，给出的搜索结果条数是一个大概值，这时我们就可以通过使用 `show table status` 或 `explain` 来**获取表的行数估算值**，执行 explain 的效率是很高的，因为它并不会真的去查询

如果是想**精确**地获取表的记录总数，那可以考虑**维护一个单独的计数器表**，在对主表进行 insert/delete 操作时，同时更新计数器表中的记录数，这样在查询时就可以直接读取计数器表中的值，避免了全表扫描，只是需要额外维护这张计数表

#### MySQL 分页有什么性能问题？怎么优化？深度分页问题是什么？

在 MySQL 中如果要做分页，一般都是用 **`select * from page order by id limit offset, size`** 这种方式来实现的，其中 **offset 是偏移量**，**size 是每页的记录数**，当偏移量非 0 时，MySQL 会**先扫描前 offset + size 条记录，然后丢弃前 offset 条记录，只返回后 size 条记录**，这是传统的分页实现方式

当 offset **很大**时，MySQL 需要**扫描和丢弃大量的记录**，导致查询效率低下，这就是**分页的性能问题**，当 offset **特别大**，MySQL 却还是要从头数、扫描/排序大量数据，把数据库拖到几秒甚至几十秒，这就叫做**深度分页问题**，解决的核心思路是**让扫描的数据量变少**，或者**干脆不要 offset**
...
LIMIT 100000, 20;   -- 第 5001 页，每页 20 条
```

执行过程是**先根据 where 条件找出所有符合的行**，**然后根据 order by 对这些行进行排序**，排序过程可能需要 filesort + 临时表，**最后再从排好序的结果中先扫描前 100020 条记录，丢弃前 100000 条，只返回后 20 条记录**，真正需要的只有 20 条记录，但为了到达这 20 条之前的位置，MySQL 需要扫描和丢弃 100000 条记录，导致查询效率低下，并且如果 order by 没有合适的索引，排序过程还会非常耗时

我们在传统 offset 分页的前提下，可以**让 where 和 order by 一定要走上索引扫**描，避免 filesort 和临时表的开销，例如上面那条 SQL 我们就可以用 `INDEX idx_target_time (user_id, created_at DESC);` 这个联合索引来覆盖，或者**限制最大页数**，不让随便深度分页，因为**很多业务其实没必要让用户翻到第 1000 页**，也可以**把一段查询改成两段，先用一个子查询把主键 id 列先查出来，然后再通过主键 id 回表取数据**，例如：
...
这样相当于**第一步只扫索引，不回表**，然后**第二步再通过主键 id 回表取数据**，减少了回表的行数，从而提升查询效率

另外就是干脆别用 offset，而是用**基于主键/时间的游标分页**，也就是**记住上一页最后一条记录的排序值**，然后**下一页从这个排序值开始往后取**，例如：
...
`:last_created_at` 和 `:last_id` 可以**用两个变量在循环内每轮都保存起来**，这样就相当于**让数据根据 id 主键或创建时间排序**，然后**分批次取出**，**将当前页的最后一条记录作为下一页的起点**，从而获得稳定的查询性能，因为**每次都能往后遍历相同规模的数据**，当然这种做法是**不能跳页**的，只能支持上一页或下一页，这是很多**无限流 / 聊天记录**场景的标准做法，例如抖音的瀑布流
