...
> 定位：本章是三主题实施与验收的**唯一**色值/字体/尺寸/动效依据，逐值对齐用户确认的 mockup（visual-style.html / final-layout-v2.html / mobile-layout.html），仅对不过 WCAG AA 的文字色做**同色相加深**修正（逐条标注「AA 修正」）。frontend-design 打磨只能在此之上微调：不得把已过 AA 的色调亮、不得偏离 token、不得为任何主题增删 DOM。
