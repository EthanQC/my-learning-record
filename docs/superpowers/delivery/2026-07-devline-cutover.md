...
1. **GSC 资产验证 + 两组前缀 410 后移除请求**：即上文「未实测项」第 1 条的两个 GSC 操作（提交新 sitemap、移除 `/murmurs*` 前缀）。**2026-07-16 更新（站点侧已全部备好，验证卡在外部网络约束）**：站点侧已把三种验证方式全部就位——① meta 标签（`NEXT_PUBLIC_GSC_VERIFICATION` 已配入仓库 Variables，随 `6f342bd` 部署，线上已渲染）；② HTML 文件（`/googleaf3e76e84c749c1b.html`，线上 200）；③ DNS TXT 记录值已从 GSC 取出。实测 controller 用真实浏览器登录 GSC 发起验证，**HTML 文件方式两次均失败，失败原因 = `The connection to your server timed out`**（Google 验证器从海外经 GFW 访问阿里云深圳服务器超时；诊断：无 AAAA 记录排除 IPv6 问题，但 TLS 握手实测 1.4–4.6s，叠加国际链路触发 Google 超时）。meta/文件方式都依赖 Google 抓取服务器、走同一条链路，同样受阻。**唯一可靠路径 = DNS TXT 验证**（Google 直接查 DNS，绕开 web 服务器可达性），但添加 TXT 记录需用户的 hichina（阿里云/万网）DNS 控制台，assistant 登录阿里云控制台会触发风控且属用户账号操作，服务器上亦无任何阿里云 CLI/AccessKey——故此步为用户人工项。~~待用户在 hichina DNS 控制台为 `qingverse.com` 加一条 TXT 记录~~ **→ 全部完成（2026-07-18）**：用户在 hichina 添加 TXT 记录（主机 `@`），`dig` 实测权威 NS/Google DNS/阿里 DNS 三方生效后，controller 用用户浏览器回 GSC 重新发起添加资产——**「Ownership auto verified」，验证方式 = Domain name provider（DNS TXT）**。随后完成两项收尾：① **sitemap 已提交**（`/sitemap.xml`，Submitted Jul 18, 2026；初始状态 "Couldn't fetch" 属提交后即时态 + 已知的 Google↔大陆服务器抓取慢，Google 会周期性重试）；② **`/murmurs` 前缀移除请求已提交**（Type: Temporarily remove URL，Starts with `https://qingverse.com/murmurs`，Requested Jul 18, 2026，Status: Processing request）——六个月屏蔽窗口内 `/murmurs*` 的 410 会让 Google 重抓时永久移除。注意：**TXT 记录与 public/ 下验证文件须保留**（删除即失去验证）。至此阶段一以来的 GSC 全部待办清零。
2. **ACR/VPS 旧 web 镜像清理 — 已全部完成（2026-07-16）**：**VPS 本地部分**已于 2026-07-14 完成（129 个过时 tag 删除，磁盘 8.79GB → 1.27GB）；**ACR 云端注册表部分**已于 2026-07-16 由 controller 用服务器 `/home/w/.docker/config.json` 的 ACR 凭据经 registry v2 API 完成——`qingverse-web` 仓库 73 个 tag 中，删除 64 个未被当前 `main` 历史可达的重写前 tag（含全部烘焙 murmurs 内容的旧镜像 + 3 个日期式旧 tag），删除前按 digest 保护清单避免误删共享层。**保留 10 个**：当前及近期在役 tag（`819ad27`/`6d586d2`/`6f342bd`/`d80181d`/`d578aae`/`fee0c52`/`f827165`/`2b0e046`/`4dc72f9`）+ `c751b0c`（§7A 回滚基线）。至此「重写前烘焙 murmurs 内容的镜像」在 VPS 本地与 ACR 云端均已清空。
