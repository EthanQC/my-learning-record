...
export default async function InternshipRecordsPage() {
  const allPosts = await getPosts();
  
  // 过滤实习记录文章
  const posts = allPosts.filter(post => 
    post.slug.includes('internship-records') || post.category === 'internship-records'
  );
    ...
    <div className="container mx-auto max-w-4xl px-4 sm:px-6 py-8 sm:py-12">
      ...
      <nav className="mb-6 text-sm">
        <ol className="flex items-center gap-2 text-gray-500">
          ...
          <li>/</li>
          <li className="text-gray-800 font-medium">实习记录</li>
        </ol>
      </nav>
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-500">实习期间的工作内容与成长</p>
      ...
      {posts.length === 0 ? (
        <Card className="text-center py-12">
          <p className="text-gray-500">暂无实习记录</p>
        </Card>
      ) : (
